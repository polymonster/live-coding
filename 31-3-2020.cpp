...
struct live_lib : public __ecs, public __dbg
{
    ...
    int on_load(live_context* ctx)
    {
        ...
        scene->lights[light].direction = normalised(vec3f(0.2f, 0.5f, 1.0f));
        scene->lights[light].type = e_light_type::dir;
        scene->lights[light].flags |= e_light_flags::shadow_map;
        scene->transforms[light].translation = vec3f::zero();
        scene->transforms[light].rotation = quat();
        ...
        for(;;)
        {
            ...
            vec3f hsv = vec3f(lerp(0.1f, 0.4f, (f32)start.x/grid_size), 1.0f, 0.5f);
    ...
    {
        static f32 p = 0.0f;
        static f32 z = base_offset.z;
        
        for(u32 i = 0; i < num_boxes; ++i)
        {
            f32 dz = floor(min(scene->transforms[i].translation.z - z, 0.0f));
            f32 s = scene->transforms[i].scale.x * 0.5f;
            f32 w = sin(-dz/200.0) * s;
